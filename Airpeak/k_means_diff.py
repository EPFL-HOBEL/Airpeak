...
"""
    ...
    """
def k_means_diff(df, timestamp, n_clusters=2, scaler = MinMaxScaler(), transformer = QuantileTransformer()):
    ...
    df_peak['status'] = KMeans(n_clusters = n_clusters, max_iter = 100000).fit_predict(X_scaled)
    # n_clusters of 2 if no steady state, otherwise 3
    label_dic = df_peak.groupby('status').agg({'diff_gd':np.mean}).rank()['diff_gd'].to_dict() #ranking clusters by concentration gradient 
    df_peak['status_label'] = df_peak['status'].map(label_dic) 
    # 0-baseline, 1-decay, 2-buildup | 0-baseline, 1-decay, 2-pleateau, 3-buildup
    df_new = df_new.merge(df_peak[['status_label',timestamp]], how='left', on = timestamp)
