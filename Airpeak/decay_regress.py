...
#by default, the whole recognized periods will be used 
    decay_result = []
    decay_list = [i for i in set(df_date['decay_group']) if i>=0] #unique decay groups excluding 0 (non-decay)
    for decay in decay_list:
        df_regress = df_date[df_date['decay_group']==decay]
        df_regress = df_regress[int(len(df_regress)*portion[0]) : int(len(df_regress)*portion[1])] ## remove heads and tails
        num = len(df_regress)
        ...
        df_regress['ln'] = -1*np.log((df_regress[pollutant]-df_regress['baseline'])/(init-base_init))
        df_regress['t_diff'] = (df_regress[timestamp] - t_init)/np.timedelta64(1, 's')/3600
        df_regress = df_regress.dropna(subset=[pollutant,'diff','diff_ma','diff_gd','diff_rhl'])
        ...
        decay_result.append({'pollutant':pollutant,'time':t_mid, 'decay_start':t_init, 'decay_end':t_end,'decay_rate':coef,'r2':r_squared,'ste':standard_error,
                            'num_of_point':num,'base_value':base_init,
                            'median_ele':median,'max_diff':maximum,'group':decay, 'method':'decay'})
